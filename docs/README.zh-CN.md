...
运行 `fnsh ui`（也支持 `fnsh web`），即可启动并打开本地 Web 界面。支持搜索和运行工具、填写参数、安装/修复/移除依赖包、管理本地扩展及检查环境。界面内嵌在可执行文件中，无需另装 Node.js。

`fnsh ui` 默认执行 `start`，确保后台服务启动后打印状态和访问地址，关闭终端不影响使用。用 `fnsh ui status` 查看状态，`fnsh ui stop` 停止服务。可用 `fnsh ui --port 8080` 指定端口，或用 `fnsh ui --no-open` 只输出访问链接；修改端口前先停止已有服务。工具以独立子页面呈现，支持中英文切换、本地文件/目录选择、枚举下拉框和布尔开关。详见[使用说明与架构](local-ui.md)。
...
FinishBit 当前沿 `v0.1.x` 系列迭代，当前版本为 `v0.1.9`，随后按 `v0.1.10` 等逐次递增。新增功能或依赖组不自动改变版本系列。核心运行时、`fnsh` CLI、托管音视频/PDF/图片/表格/文档/压缩包 Provider、本地扩展协议 v1 和 Agent Skill 已经可用。生产环境依赖首个稳定版本前的接口前，请阅读[兼容性政策](compatibility.md)。
