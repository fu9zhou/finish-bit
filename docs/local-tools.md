...
本次未发布版本在 170 个 Operation 基础上新增 **107 个，合计 277 个**。86 个新能力直接在 Go 二进制内运行，另外复用本地 PDF、图片、OCR、语音和录屏运行时。CLI 和后续 Web 适配器共用 `pkg/app` / `operation.Runner`；没有上传文件的后端、第三方处理 API 或常驻服务。
